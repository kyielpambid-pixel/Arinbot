...
# Token loaded from environment variable (.env file)
# ============================================================
...
load_dotenv()
TOKEN = os.getenv("DISCORD_TOKEN")  # <-- LAGAY YUNG TOKEN MO SA .env FILE
...
SUPPORTED_DEOBFUSCATORS = {
    ...
    "Moonsec V3": {
        ...
        "patterns": (
            r"moonsec\s*v3",
            r"protected\s+with\s+moonsec\s*v3",
            r"moon_api",
            r"moon_encrypt",
        ),
    },
    ...
    "LuaObfuscator Alpha (Chaotic Good)": {
        ...
        "patterns": (
            r"\bferib\b",
            r"_welcome\s+to\s+luaobfuscator\.com",
    ...
    "LuaObfuscator Alpha (Chaotic Evil)": {
        ...
        "patterns": (
            ...
            r"integrity\s+protected",
            r"_Welcome\s+to\s+LuaObfuscator\.com",
            r'["\'"]LOL![0-9A-Fa-f2-9Q]{20,}["\']',
            r"local\s+function\s+v23\s*\(",
        ),
    },
    "LuaObfuscator Alpha (Obfuscate V1)": {
        ...
        "patterns": (
            r"\bLOL!0E3Q",
            r"\blocal\s+v0\s*=\s*tonumber\b.*?\bLOL!",
        ),
    },
...
for _luraph_name, _luraph_patterns in LURAPH_VERSION_PATTERNS.items():
    SUPPORTED_DEOBFUSCATORS[_luraph_name] = {
        "endpoint": None,
        "patterns": _luraph_patterns,
    }
...
async def upload_to_pastefy(content: str, title: str) -> str:
    return "Pastefy upload not configured."
...
async def get_script_content_from_input(file, url):
    return await load_deobfuscation_input(file, url)
...
def _luraph_try_decrypt_blob(blob: bytes, seed: int, version: str) -> list:
    ...
    if "15.0" in version:
        ...
        try:
            candidates.append(zlib.decompress(xored))
        except Exception:
            pass
        try:
    ...
    while index < len(value):
        ...
        if escaped == "x" and index + 2 < len(value):
            ...
            if re.fullmatch(r"[0-9a-fA-F]{2}", hexadecimal):
                output.append(chr(int(hexadecimal, 16)))
                index += 3
                continue
...
def _decode_luaobfuscator_blob(value):
    ...
    for index in range(0, len(encoded) - 1, 2):
        ...
        if pair[1] == "Q":
            ...
            continue
        try:
            decoded.extend(bytes([int(pair, 16)]) * repeat_count)
        except ValueError:
            return b""
        repeat_count = 1
...
def _extract_luaobfuscator_blobs(content):
    ...
    for match in pattern.finditer(content):
        ...
        decoded = _decode_luaobfuscator_blob(encoded)
        if decoded:
...
def _looks_like_lua_source(value):
    if not value or len(value) < 8:
        return False
    printable_ratio = sum(character in "\n\r\t" or ord(character) >= 32 for character in value) / len(value)
    if printable_ratio < 0.96:
        return False
    return bool(
...
def _recover_embedded_lua_source(blobs):
    ...
    for blob in blobs:
        ...
        for nested in re.findall(rb"LOL![A-Za-z0-9]+", blob):
            ...
            if nested_blob:
                candidates.append(nested_blob.decode("utf-8", errors="replace"))
    for candidate in candidates:
        candidate = remove_lua_comments(candidate).strip()
        if _looks_like_lua_source(candidate):
            cleaned = "".join(character for character in candidate if character in "\n\r\t" or ord(character) >= 32)
...
def _recover_lua_from_constants(content, blobs):
    ...
    useful = [value for value in printable if value not in ignored and not value.startswith("LOL!") and len(value) <= 4096 and not re.fullmatch(r"[\W_]+", value)]
    if not useful:
        return None
    lines = ["-- Recovered LuaObfuscator VM constants."]
    for index, value in enumerate(useful, 1):
        lines.append(f"local recovered_{index} = {_lua_literal(value)}")
    lines.extend(["", "return {", *[f"    recovered_{index}," for index in range(1, len(useful) + 1)], "}"])
    return "\n".join(lines)
...
def _deobfuscate_chaotic_evil_blob(content: str):
    ...
    for blob_data in blobs:
        ...
        for ki, klen, key_bytes in constants:
            if klen < 4 or klen > 128:
                continue
            for di, dlen, data_bytes in constants:
                if di == ki or dlen <= klen:
                    continue
                decrypted_raw = _xor_decrypt_v7(data_bytes, key_bytes)
                try:
                    text = decrypted_raw.decode("utf-8")
                except UnicodeDecodeError:
                    try:
                        text = decrypted_raw.decode("latin-1")
                    except Exception:
                        continue
                ...
                if re.match(r"https?://", stripped):
                    url_results.append(stripped)
                elif _looks_like_lua_source(stripped):
                    lua_results.append(stripped)
        if lua_results:
            return max(lua_results, key=len)
        if url_results:
            url = url_results[0]
...
def _luaobfuscator_mode_scores(content):
    ...
    if re.search(r'local\s+\w+\s*=\s*string\.char\s*;?\s*local\s+\w+\s*=\s*string\.byte', content, re.IGNORECASE):
        scores["chaotic_good"] += 2
        scores["chaotic_evil"] += 2
    if re.search(r'local\s+\w+\s*=\s*string\.char.*?local\s+\w+\s*=\s*bit32\s+or\s+bit', content, re.IGNORECASE | re.DOTALL):
        scores["chaotic_good"] += 2
        scores["chaotic_evil"] += 3
    if re.search(r'local\s+function\s+\w+\s*\(.*?bxor', content, re.IGNORECASE | re.DOTALL):
        scores["chaotic_good"] += 2
        scores["chaotic_evil"] += 1
    if "integrity protected" in lower_content:
        scores["chaotic_evil"] += 4
    if re.search(r'\b\w+\s*\(\s*["\']\\d+', content):
        scores["chaotic_good"] += 4
    if re.search(r'local\s+v0\s*=\s*tonumber\s*;?\s*local\s+v1\s*=\s*string\.byte', content, re.IGNORECASE):
        scores["obfuscate_old"] += 2
        scores["obfuscate_v1"] += 2
    if re.search(r'\b\w+\s*\(\s*["\']LOL!0E3Q', content):
        scores["obfuscate_v1"] += 5
    if re.search(r'\b\w+\s*\(\s*["\']LOL!153Q', content):
        scores["obfuscate_old"] += 5
    return scores
...
def _luaobfuscator_detect_mode(content):
    ...
    if best_mode in ("chaotic_good", "chaotic_evil") and "LOL!" in content:
        if scores["chaotic_evil"] > scores["chaotic_good"]:
            return "chaotic_evil"
    return best_mode

def _luaobfuscator_candidate_matches(content, mode):
    if mode == "chaotic_good":
        return bool(re.search(r'\b\w+\s*\(\s*["\']\\d+', content) and not re.search(r'["\']LOL![0-9A-Fa-f2-9Q]+["\']', content))
    if mode == "chaotic_evil":
        return bool("integrity protected" in content.lower() or re.search(r'\b\w+\s*\(\s*["\']LOL!', content))
    if mode == "obfuscate_v1":
        return bool(re.search(r'\b\w+\s*\(\s*["\']LOL!0E3Q', content) or (re.search(r'\bLOL!', content) and re.search(r'\blocal\s+v0\s*=\s*tonumber\b', content, re.IGNORECASE) and not re.search(r'LOL!153Q', content)))
    if mode == "obfuscate_old":
        return bool(re.search(r'\b\w+\s*\(\s*["\']LOL!153Q', content) or (re.search(r'\bLOL!', content) and re.search(r'\blocal\s+v0\s*=\s*tonumber\b', content, re.IGNORECASE) and not re.search(r'LOL!0E3Q', content)))
    return False
...
def _deobfuscate_luaobfuscator_chaotic_good(content):
    ...
    matches = [match for pattern in patterns for match in pattern.finditer(content)]
    if not matches:
        return None
    replacements = {}
    for match in matches:
        ...
        if not encrypted or not key:
            continue
        decoded = bytes(value ^ key[index % len(key)] for index, value in enumerate(encrypted, 1)).decode("utf-8", errors="replace")
...
def _build_luaobfuscator_vm_output(content, mode, blobs):
    ...
    if recovered_constants:
        return recovered_constants
    cleaned = remove_lua_comments(content).strip()
    return rename_obfuscated_lua_identifiers(cleaned).strip()

def _luaobfuscator_deobfuscate(content, mode):
    if not _luaobfuscator_candidate_matches(content, mode):
        return None
    if mode == "chaotic_good":
    ...
    blobs = _extract_luaobfuscator_blobs(content)
    if blobs:
        return _build_luaobfuscator_vm_output(content, mode, blobs)
...
async def _try_luaobfuscator_modes(content, preferred_mode=None):
    ...
    modes.sort(key=lambda candidate: scores.get(candidate, 0), reverse=True)
    if preferred_mode in modes:
        modes.remove(preferred_mode)
...
def detect_supported_deobfuscator(content: str):
    if IS_MOONVEIL_V2022 is not None:
        try:
            if IS_MOONVEIL_V2022(content):
                return "MoonVeil 2.0.22"
        except Exception as error:
    ...
    for name, detector in SUPPORTED_DEOBFUSCATORS.items():
        ...
        if not score:
            continue
        if name.startswith("Luraph"):
            luraph_matches.append((score, name))
        else:
...
async def try_deobfuscator(input_path: str, content: str, service_name: str) -> str:
    ...
    if service_name == "MoonVeil 2.0.22":
        if MOONVEIL_DECODER is None:
            raise RuntimeError("The MoonVeil decoder module is not available.")
...
async def try_all_deobfuscators(input_path, content, detected_name, timeout_seconds, on_attempt_failure=None):
    if detected_name == "MoonVeil 2.0.22":
        ...
        candidates = list(SUPPORTED_DEOBFUSCATORS)
    if detected_name in candidates:
        candidates.remove(detected_name)
        candidates.insert(0, detected_name)
    failures = []
    ...
    for service_name in candidates:
        ...
        try:
            ...
            failure = f"{service_name}: timed out"
        except Exception as error:
            failure = f"{service_name}: {error}"
        await asyncio.sleep(max(0.0, step_timeout - (time.monotonic() - attempt_started_at)))
        failures.append(failure)
        if on_attempt_failure is not None:
            await on_attempt_failure(len(failures))
...
def deobfuscation_preview(code, line_limit=5, character_limit=900):
    ...
    if len(preview) > character_limit:
        preview = preview[:character_limit].rstrip() + "\n..."
    return f"```lua\n{preview}\n```"
...
async def send_deobfuscation_failure(interaction, output, detected=False, file_error="Good", fallback_output=None, fallback_obfuscator=None):
    ...
    embed.set_footer(text="Deobfuscation Failed | SigmaBot")
    if isinstance(fallback_output, str) and fallback_output.strip():
...
async def run_deobfuscation_step(awaitable, timeout_seconds=DEOBF_STEP_INTERVAL_SECONDS):
    if timeout_seconds <= 0:
        raise asyncio.TimeoutError("Deobfuscation step timed out.")
...
def check_deobfuscated_lua(code):
    source = str(code or "")
    if not source.strip():
        return ["The deobfuscated output is empty."]
    ...
    if lua_compiler:
        ...
        try:
            ...
            return ["Lua syntax check timed out."]
        except OSError:
            pass
        finally:
            if temporary_path:
    ...
    for character in source:
        if character in opening:
            ...
            if not stack or stack[-1] != pairs[character]:
                errors.append(f"Unexpected closing delimiter: {character}")
            else:
                stack.pop()
    if stack:
...
async def run_auto_deobfuscation(interaction, file=None, url=None, confirmed=False):
    ...
    try:
        ...
        if size is not None and size >= LARGE_DEOBF_FILE_BYTES and not confirmed:
            ...
            embed.set_footer(text="SigmaBot | Large File Warning!")
        ...
        embed = discord.Embed(title="Deobfuscation Success!", description="Deobfuscation Success! enjoy lol, See status below", color=None)
        ...
        embed.set_footer(text=f"Powered by SigmaBot | Request From {interaction.user}")
        ...
        if confirmed:
            clear_large_deobf_timeout(interaction.user.id)
        try:
            ...
            if confirmed:
                await send_deobf_timeout_removed_notice(interaction)
    finally:
        if progress_message is not None:
            await delete_deobfuscation_progress(progress_message)
        if input_path:
...
@client.tree.command(name="deobfuscate", description="Detect and deobfuscate a supported Lua obfuscator.")
...
@client.tree.command(name="deobfuscate_luraph", description="Deobfuscate a Luraph-protected Lua script.")
@app_commands.describe(
...
async def deobfuscate_luraph(
    ...
    if version != "auto":
        ...
        try:
            ...
            setattr(_module, "detect_supported_deobfuscator", _original)
    else:
        await run_auto_deobfuscation(interaction, file=file, url=url)
